...
This document contains annotated YAML definitions for monitoring and controlling a [[https://mybox.eco/produkty-kategorie/nabijeci-stanice/][MyBox]] wallbox in Home Assistant. Details can be found in this [[https://blog.lhotka.name/posts/mybox-home-assistant/][blog post]].

The generated YAML configuration files can be included in Home Assistant configuration by adding the following lines to the `configuration.yaml` file:
...
*IMPORTANT*: MQTT topics in YAML definitions use placeholders DEVICE_ID and
ROOT_TOPIC. These have to be replaced by the actual device ID and MQTT root topic configured in [[https://cloud.mybox.pro/admin-panel/devices-management][MyBox portal]].
